...
export class PokemonResolver implements Resolve<Observable<Pokemon.IPokemon>> {
    ...
    public resolve(route: ActivatedRouteSnapshot): Observable<Pokemon.IPokemon> {
        const pokemonName = route.paramMap.get('name');
        return this.store.select(PokemonState.getPokemonDetailsByName(pokemonName))
            .pipe(
                tap((pokemon: Pokemon.IPokemon) => {
                    if (pokemon === null || pokemon === undefined) {
                        this.store.dispatch(new GetPokemonDetails({ name: pokemonName }));
                    }
                }),
